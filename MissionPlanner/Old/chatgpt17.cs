...
//
// This version:
// - Moves Save/Load/Hide buttons to the bottom of the tree window
// - Reduces title widths by ~1/4 (uses ~75% of available row width)
// - Persists skin + window positions (UI.cfg)
// - Styles rebuild on skin toggle
// - Drag window from anywhere (DragWindow at end of each window draw)
...
namespace MissionPlanner
{
    ...
    public class HierarchicalStepsWindow : MonoBehaviour
    {
        ...
        // Updated EnsureStyles (your version)
        ...
        private void DrawTreeWindow(int id)
        {
            ...
            GUILayout.Space(6);
            GUILayout.Label("Only the step titles are shown below. Double-click a title or click “Edit…” to modify data in a separate window. Use ▲ ▼ ⤴ ⤵ Move… ⊕ Dup ✖ to manage hierarchy.", _hintLabel);
        ...
        private void DrawNodeRow(StepNode node, int depth)
        {
            ...
            // Indent & fold
            float indent = 12f + 18f * depth;
            GUILayout.Space(indent);

            const float foldWidth = 24f;
            if (node.Children.Count == 0)
            {
                GUILayout.Label("·", GUILayout.Width(foldWidth));
            }
            else
            {
                string sign = node.Expanded ? "−" : "+";
                if (GUILayout.Button(sign, GUILayout.Width(foldWidth))) node.Expanded = !node.Expanded;
            }

            // --- Title width reduction (~25%) ---
            // Reserve space for right-side controls (sum of button widths + margins).
            // Buttons: ▲(28) ▼(28) ⤴(28) ⤵(28) Move(54) Edit(54) Dup(40) ⊕(26) ✖(28) => 314
            // Add a margin buffer (~40).
            const float controlsWidth = 314f + 40f;
            float available = Mathf.Max(120f, _treeRect.width - indent - foldWidth - controlsWidth - 12f);
            float titleWidth = Mathf.Clamp(available * 0.75f, 120f, available); // 75% of prior space
        ...
        // Moves / hierarchy
        private void MoveUp(StepNode n) { var list = (n.Parent == null) ? _roots : n.Parent.Children; int idx = list.IndexOf(n); if (idx > 0) { list.RemoveAt(idx); list.Insert(idx - 1, n); } }
        private void MoveDown(StepNode n) { var list = (n.Parent == null) ? _roots : n.Parent.Children; int idx = list.IndexOf(n); if (idx >= 0 && idx < list.Count - 1) { list.RemoveAt(idx); list.Insert(idx + 1, n); } }
        private void Promote(StepNode n)
        {
            if (n.Parent == null) return;
            var parent = n.Parent; var oldList = parent.Children; int oldIdx = oldList.IndexOf(n);
            if (oldIdx >= 0) oldList.RemoveAt(oldIdx);
            var newList = (parent.Parent == null) ? _roots : parent.Parent.Children;
            int parentIdx = newList.IndexOf(parent); int insertAt = (parentIdx >= 0) ? parentIdx + 1 : newList.Count;
            newList.Insert(insertAt, n); n.Parent = parent.Parent;
        }
        private void Demote(StepNode n)
        {
            var list = (n.Parent == null) ? _roots : n.Parent.Children; int idx = list.IndexOf(n); if (idx <= 0) return;
            StepNode prev = list[idx - 1]; list.RemoveAt(idx); prev.Children.Add(n); n.Parent = prev; prev.Expanded = true;
        }
        private void Delete(StepNode n) { if (n.Parent == null) _roots.Remove(n); else n.Parent.Children.Remove(n); if (_selectedNode == n) _selectedNode = null; if (_detailNode == n) _detailNode = null; }
        private void Duplicate(StepNode n)
        {
            ...
            var list = (n.Parent == null) ? _roots : n.Parent.Children; int idx = list.IndexOf(n);
            list.Insert(Mathf.Clamp(idx + 1, 0, list.Count), cloned); cloned.Parent = n.Parent;
        }

        // Move dialog
        private void OpenMoveDialog(StepNode node)
        {
            _moveNode = node; _moveTargetParent = null; _showMoveDialog = true;
        ...
        private void DrawMoveDialogWindow(int id)
        {
            ...
            if (GUILayout.Toggle(rootChosen, "⟂ Root", HighLogic.Skin.toggle, GUILayout.Width(120))) _moveTargetParent = null;
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();
            ...
            GUILayout.Space(8);
            GUILayout.BeginHorizontal();
            if (GUILayout.Button("OK", GUILayout.Width(100))) { MoveToParent(_moveNode, _moveTargetParent); CloseMoveDialog(); }
        ...
        private void DrawMoveTargetRecursive(StepNode node, int depth, StepNode moving)
        {
            ...
            if (GUILayout.Toggle(chosen, label, HighLogic.Skin.toggle)) _moveTargetParent = node;
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();
            foreach (var c in node.Children) DrawMoveTargetRecursive(c, depth + 1, moving);
        }

        private void CloseMoveDialog() { _showMoveDialog = false; _moveNode = null; _moveTargetParent = null; }

        private void MoveToParent(StepNode n, StepNode newParent)
        {
            if (n == null) return;
            if (newParent == n || IsDescendant(n, newParent)) return;
            var oldList = (n.Parent == null) ? _roots : n.Parent.Children; oldList.Remove(n);
            if (newParent == null) { _roots.Add(n); n.Parent = null; }
            else { newParent.Children.Add(n); n.Parent = newParent; newParent.Expanded = true; }
        }
        ...
        private void DrawDetailWindow(int id)
        {
            ...
            switch (s.stepType)
            {
                case StepType.toggle:
                    ...
                    GUILayout.EndHorizontal();
                    break;
                case StepType.intNotGreaterThan: IntField("≤ (int)", ref s.intNotGreaterThan); break;
                case StepType.intNotLessThan: IntField("≥ (int)", ref s.intNotLessThan); break;
                case StepType.floatNotGreaterThan: FloatField("≤ (float)", ref s.floatNotGreaterThan); break;
                case StepType.floatNotLessThan: FloatField("≥ (float)", ref s.floatNotLessThan); break;
                case StepType.intRange:
                    IntRangeFields(ref s.minIntRange, ref s.maxIntRange);
                    if (s.minIntRange > s.maxIntRange) GUILayout.Label("Warning: minIntRange is greater than maxIntRange.", _tinyLabel);
                    break;
                case StepType.floatRange:
                    FloatRangeFields(ref s.minFloatRange, ref s.maxFloatRange);
                    if (s.minFloatRange > s.maxFloatRange) GUILayout.Label("Warning: minFloatRange is greater than maxFloatRange.", _tinyLabel);
                    break;
            }

            GUILayout.Space(6);
            GUILayout.BeginHorizontal();
            if (GUILayout.Button("Save", GUILayout.Width(100))) { ScreenMessages.PostScreenMessage($"Saved step “{s.title}”.", 2f, ScreenMessageStyle.UPPER_LEFT); TrySaveToDisk(); }
        ...
        private void SeedSample()
        {
            ...
            _roots.Add(preflight); _roots.Add(ascent);
        }

        // Missions I/O
        private string GetCurrentSaveName() => HighLogic.SaveFolder ?? "UnknownSave";
        private static string SanitizeForFile(string s) { if (string.IsNullOrEmpty(s)) return "Unnamed"; foreach (char c in Path.GetInvalidFileNameChars()) s = s.Replace(c.ToString(), "_"); return s.Trim(); }
        ...
        private void TrySaveToDisk()
        {
            ...
            if (File.Exists(path))
            {
                _pendingSavePath = path; _pendingSaveMission = mission; _showOverwriteDialog = true;
                var mp = Input.mousePosition;
            ...
            else { TrySaveToDisk_Internal(overwriteOk: true); }
        }
        ...
        private bool TrySaveToDisk_Internal(bool overwriteOk)
        {
            try
            {
                ...
                var list = new ConfigNode(SAVE_LIST_NODE); root.AddNode(list);
                foreach (var r in _roots) list.AddNode(r.ToConfigNodeRecursive());
            ...
            catch (Exception ex) { Debug.LogError($"[MissionPlanner] Save failed: {ex}"); ScreenMessages.PostScreenMessage("Save failed (see log).", 3f, ScreenMessageStyle.UPPER_LEFT); return false; }
        }
        ...
        private bool TryLoadFromDisk(string fullPath)
        {
            try
            {
                ...
                foreach (var n in list.GetNodes("NODE")) { var node = StepNode.FromConfigNodeRecursive(n); node.Parent = null; newRoots.Add(node); }
                _roots.Clear(); _roots.AddRange(newRoots); ReparentAll();
                return true;
            }
            catch (Exception ex) { Debug.LogError($"[MissionPlanner] Load failed: {ex}"); ScreenMessages.PostScreenMessage("Load failed (see log).", 3f, ScreenMessageStyle.UPPER_LEFT); return false; }
        }

        private bool TryAutoLoadMostRecentForCurrentSave()
        {
            string save = GetCurrentSaveName(); var list = GetAllMissionFiles();
            MissionFileInfo best = default; bool found = false;
            foreach (var mf in list) if (mf.SaveName.Equals(save, StringComparison.OrdinalIgnoreCase)) { if (!found || mf.LastWriteUtc > best.LastWriteUtc) { best = mf; found = true; } }
            if (found) { _missionName = best.MissionName; return TryLoadFromDisk(best.FullPath); }
            return false;
        }

        // Load dialog + listing
        private struct MissionFileInfo { public string FullPath; public string SaveName; public string MissionName; public DateTime LastWriteUtc; }
        ...
        private List<MissionFileInfo> GetAllMissionFiles()
        {
            ...
            try
            {
                ...
                foreach (var file in Directory.GetFiles(dir, "*" + SAVE_FILE_EXT, SearchOption.TopDirectoryOnly))
                {
                    ...
                    if (idx > 0 && idx < name.Length - 2) { save = name.Substring(0, idx); mission = name.Substring(idx + 2); }
                    else
                    {
                    ...
                    results.Add(new MissionFileInfo { FullPath = file, SaveName = save, MissionName = mission, LastWriteUtc = File.GetLastWriteTimeUtc(file) });
                }
            }
            catch (Exception ex) { Debug.LogError($"[MissionPlanner] Listing missions failed: {ex}"); }
            return results;
        }

        private void OpenLoadDialog()
        {
            _showLoadDialog = true; _loadShowAllSaves = false; _loadList = GetAllMissionFiles();
            var mp = Input.mousePosition;
        ...
        private void DrawLoadDialogWindow(int id)
        {
            ...
            {
                if (!_loadShowAllSaves && !mf.SaveName.Equals(curSave, StringComparison.OrdinalIgnoreCase)) continue;
                ...
                {
                    if (TryLoadFromDisk(mf.FullPath)) { _missionName = mf.MissionName; ScreenMessages.PostScreenMessage($"Loaded mission “{_missionName}”.", 2f, ScreenMessageStyle.UPPER_LEFT); _showLoadDialog = false; }
                    else ScreenMessages.PostScreenMessage("Failed to load mission (see log).", 2f, ScreenMessageStyle.UPPER_LEFT);
                }
                if (GUILayout.Button("Delete", GUILayout.Width(70)))
                {
                    _deleteTarget = mf; _showDeleteConfirm = true;
                    var mp = Input.mousePosition;
        ...
        private void DrawDeleteDialogWindow(int id)
        {
            ...
            {
                try { if (File.Exists(_deleteTarget.FullPath)) File.Delete(_deleteTarget.FullPath); ScreenMessages.PostScreenMessage("Mission deleted.", 2f, ScreenMessageStyle.UPPER_LEFT); }
                catch (Exception ex) { Debug.LogError($"[MissionPlanner] Delete failed: {ex}"); ScreenMessages.PostScreenMessage("Delete failed (see log).", 2f, ScreenMessageStyle.UPPER_LEFT); }
                finally { _showDeleteConfirm = false; _loadList = GetAllMissionFiles(); }
            }
            if (GUILayout.Button("Cancel", GUILayout.Width(100))) _showDeleteConfirm = false;
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();
        ...
        private void DrawOverwriteDialogWindow(int id)
        {
            ...
            GUILayout.Space(12);
            GUILayout.BeginHorizontal();
            if (GUILayout.Button("Overwrite", GUILayout.Width(120))) { _showOverwriteDialog = false; TrySaveToDisk_Internal(overwriteOk: true); }
            if (GUILayout.Button("Auto-Increment & Save", GUILayout.Width(180)))
            {
                string next = GetAutoIncrementName(_pendingSaveMission);
                _missionName = next; _pendingSaveMission = null; _pendingSavePath = null; _showOverwriteDialog = false; TrySaveToDisk_Internal(overwriteOk: true);
            }
            if (GUILayout.Button("Cancel", GUILayout.Width(100))) { _showOverwriteDialog = false; _pendingSaveMission = null; _pendingSavePath = null; }
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();
        ...
        private string GetAutoIncrementName(string baseName)
        {
            ...
            string save = GetCurrentSaveName(); string name = baseName; int n = 2;
            while (File.Exists(GetSaveFileAbsolute(save, name)))
            {
                string stripped = baseName; int p = stripped.LastIndexOf('('); int q = stripped.LastIndexOf(')');
                if (p >= 0 && q == stripped.Length - 1)
                {
                    string inside = stripped.Substring(p + 1, q - p - 1);
                    if (int.TryParse(inside, out int parsed)) { stripped = stripped.Substring(0, p).TrimEnd(); n = parsed + 1; }
                }
                name = $"{stripped} ({n})"; n++;
            }
            return name;
        ...
        private void OnAppLauncherDestroyed() { if (_appButton != null && ApplicationLauncher.Instance != null) ApplicationLauncher.Instance.RemoveModApplication(_appButton); _appButton = null; }
        ...
        private void SetButtonIcon(Texture2D tex) { if (_appButton != null && tex != null) _appButton.SetTexture(tex); }
        private Texture2D MakeSolidTexture(int w, int h, Color c) { var t = new Texture2D(w, h, TextureFormat.ARGB32, false); var px = new Color[w * h]; for (int i = 0; i < px.Length; i++) px[i] = c; t.SetPixels(px); t.Apply(); return t; }

        // UI settings persistence
        private string GetUIFileAbsolute() => Path.Combine(KSPUtil.ApplicationRootPath, "GameData", SAVE_MOD_FOLDER, UI_FILE_NAME);
        private static ConfigNode RectToNode(string name, Rect r) { var n = new ConfigNode(name); n.AddValue("x", r.x); n.AddValue("y", r.y); n.AddValue("w", r.width); n.AddValue("h", r.height); return n; }
        private static Rect NodeToRect(ConfigNode n, Rect fallback)
        {
            ...
            float.TryParse(n.GetValue("x"), out x); float.TryParse(n.GetValue("y"), out y); float.TryParse(n.GetValue("w"), out w); float.TryParse(n.GetValue("h"), out h);
        ...
        private void SaveUISettings()
        {
            ...
            catch (Exception ex) { Debug.LogError($"[MissionPlanner] SaveUISettings failed: {ex}"); }
        }
        private void LoadUISettings()
        {
            ...
            catch (Exception ex) { Debug.LogError($"[MissionPlanner] LoadUISettings failed: {ex}"); }
        }
    }
}
#endif