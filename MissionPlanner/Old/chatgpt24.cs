...
// - "New" mission button opens name prompt; clears tree (optionally adds sample root step) without saving yet
// - Auto-save when closing/switching details window; save indicator (blue ✓ on success, red ✗ on errors)
...
namespace MissionPlanner
{
    ...
    {
        // ---- Windows ----
        private Rect _treeRect = new Rect(220, 120, 820, 600);
        private Rect _detailRect = new Rect(820, 160, 540, 540);
        ...
        private Rect _saveAsRect = new Rect(740, 200, 440, 190); // slightly taller for "New" toggle

        private int _treeWinId, _detailWinId, _moveWinId, _loadWinId, _overwriteWinId, _deleteWinId, _partWinId, _saveAsWinId;
        ...
        public void OnGUI()
        {
            ...
            if (_showSaveAs)
            {
                _saveAsRect = GUILayout.Window(
                    ...
                    "Save As…",
                    GUILayout.MinWidth(380), GUILayout.MinHeight(160)
                );
            }
        }
        ...
        private void DrawTreeWindow(int id)
        {
            ...
            // --- Bottom bar: New / Save / Save As… / Load / Hide + Save indicator ---
            GUILayout.Space(6);
            GUILayout.BeginHorizontal();

            if (GUILayout.Button("New", GUILayout.Width(90)))
            {
                // Open Save As dialog in "creating new" mode; don't touch disk yet
                    ...
                    if (HighLogic.LoadedSceneIsFlight && FlightGlobals.ActiveVessel != null)
                    {
                        ...
                        if (GUILayout.Button("Validate vs Vessel", GUILayout.Width(160)))
                        {
                            try
                            {
                                ...
                                bool ok = crew >= s.minIntRange && crew <= s.maxIntRange;
                                string msg = ok
        ...
        private void OpenSaveAsDialog()
        {
            if (_creatingNewMission)
            {
                ...
                _saveAsName = string.IsNullOrWhiteSpace(_saveAsName) ? "New Mission" : _saveAsName.Trim();
            }
            else
            {
                // default to current name for Save As…
                _saveAsName = string.IsNullOrWhiteSpace(_missionName) ? "Untitled Mission" : _missionName.Trim();
            }
        ...
        private void DrawSaveAsDialogWindow(int id)
        {
            ...
            if (GUILayout.Button("OK", GUILayout.Width(100)))
            {
                string proposed = string.IsNullOrWhiteSpace(_saveAsName) ? "Unnamed" : _saveAsName.Trim();
                ...
                if (_creatingNewMission)
                {
                    ...
                    _showSaveAs = false;
                    // Hint: user can press Save to write it
                }
                else
                {
