...
namespace MissionPlanner
{
    ...
    public class RecursiveCollapsibleList : MonoBehaviour
    {
        ...
        private const float DoubleClickThreshold = 0.30f;
        ...
        private Node _dragTarget = null;
        private DropKind _dragKind = DropKind.None;
        private float _hoverStart = 0f;
        private Node _hoverNode = null;
        private const float AutoExpandDelay = 0.6f;
        ...
        private const float DragThresholdSqr = 6f * 6f;
        private Vector2 _mouseDownPosScreen;
        private bool _pendingClick;
        private int _activeRowControl;
        private const string DragHandleGlyph = "⋮⋮";

        // Modal drag shield flag
        ...
        public void Update()
        {
            ...
            // Safety net: finalize drag if mouse is released outside GUI
            if ((_dragActive || _dragNode != null) && !Input.GetMouseButton(0))
            {
                if (_dragNode != null && _dragTarget != null && _dragKind != 0)
        ...
        public void OnGUI()
        {
            ...
            DrawDragVisuals();
            DrawDragShield(); // ensures MouseUp is caught even under window
        }

        // ---------- Modal shield ----------
        private void DrawDragShield()
        {
            if (_dragActive || _dragNode != null)
            {
                var e = Event.current;
                Rect full = new Rect(0, 0, Screen.width, Screen.height);
                // Invisible overlay that soaks events while dragging
                GUI.Box(full, GUIContent.none, GUIStyle.none);
        ...
        private void DrawTreeWindow(int id)
        {
            ...
            GUILayout.Space(6);
            GUILayout.Label("Drag by the ⋮⋮ handle. TOP = before, BOTTOM = after, MIDDLE = as child.", HighLogic.Skin.label);
        ...
        private void DrawNodeRecursive(Node node, int depth)
        {
            ...
            // Label (select / double-click only)
            ...
            HandleRowMouse(node, handleRect, labelRect, rowScreenRect, handleScreenRect, labelScreenRect);
        ...
        private void HandleRowMouse(Node node, Rect handleGuiRect, Rect labelGuiRect,
                                    Rect rowScreenRect, Rect handleScreenRect, Rect labelScreenRect)
        {
            var e = Event.current;
            ...
            switch (e.rawType)
            {
                case EventType.MouseDown:
                    if (e.button == 0 && handleScreenRect.Contains(mouseScreen))
                    {
                        ...
                        _hoverNode = null;
                        _hoverStart = 0f;
                        GUI.BringWindowToFront(_treeWinId);
                        _dragActive = true;
                        e.Use();
                    }
                    break;
                ...
                case EventType.MouseUp:
                    // Label single/double click behavior (no drag capture from label)
        ...
        private void DrawDetailWindow(int id)
        {
            ...
            GUILayout.Label(_detailNode.Name);
            GUILayout.FlexibleSpace();
