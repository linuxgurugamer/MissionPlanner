...
namespace MissionPlanner
{
    ...
    // KSP1 (Unity IMGUI) – recursive tree with reliable drag & drop, AppLauncher button,
    // double-click details window, per-node add dialog, hover-to-expand, drop before/after/as-child.
    //
    // Build against KSP1 assemblies; drop the DLL into GameData/YourMod/Plugins.
    // Optional toolbar icons (no file extension in path):
    ...
    namespace YourModNamespace
    {
        ...
        public class RecursiveCollapsibleList : MonoBehaviour
        {
            ...
            // ---- Styles ----
            private GUIStyle _labelLikeButton;
            private void EnsureStyles()
            {
                if (_labelLikeButton != null) return;
                _labelLikeButton = new GUIStyle(HighLogic.Skin.button)
                {
            ...
            private Node _dragTarget = null;          // row we're hovering
            ...
            private void DrawTreeWindow(int id)
            {
                ...
                GUILayout.Space(6);
                GUILayout.Label("Drag by the label. Drop at TOP = before, BOTTOM = after, MIDDLE = as child. Double-click opens details. ⊕ adds a child.", HighLogic.Skin.label);
            ...
            private void DrawNodeRecursive(Node node, int depth)
            {
                ...
                // We'll collect per-control rects as we draw, then union them for a full-row hitbox
                Rect foldRect = default, textRect = default, addRect = default;
                ...
                // LABEL drawn as Label (not Button) so it won't swallow MouseDown → enables drag start
                GUILayout.Label(node.Name, _labelLikeButton, GUILayout.ExpandWidth(true));
                textRect = GUILayoutUtility.GetLastRect();

                // Right-side add child button
                ...
                // Full-row rect = union of the three pieces (fold + text + add)
                Rect rowRect = UnionRects(UnionRects(foldRect, textRect), addRect);

                // Mouse logic (double-click, drag start, drop targeting)
                HandleRowMouse(node, rowRect);
            ...
            private void HandleRowMouse(Node node, Rect rowRect)
            {
                var e = Event.current;

                // -------- Double-click (on MouseUp over the row) --------
                if (e.type == EventType.MouseUp && e.button == 0 && rowRect.Contains(e.mousePosition))
                {
                    ...
                    // Single click: toggle if branch, open details if leaf
                ...
                // -------- Drag start (LEFT mouse) --------
                // Label is a Label (not Button), so MouseDown reaches us → reliable drag start.
                if (_dragNode == null && e.type == EventType.MouseDown && e.button == 0 && rowRect.Contains(e.mousePosition))
                {
                    ...
                    _hoverNode = null;
                    _hoverStart = 0f;
                    GUI.FocusWindow(_treeWinId); // helps stability inside scroll view
                    e.Use();
                    return;
                }

                // -------- While dragging: compute drop target & mode --------
                if (_dragNode != null && e.type == EventType.MouseDrag)
                {
                    if (rowRect.Contains(e.mousePosition) && !IsSameOrDescendant(node, _dragNode))
                    {
                        _dragTarget = node;
                        _hoverNode = node;
            ...
            private void DrawDragVisuals()
            {
                ...
                // Draw highlight hint near cursor
                ...
                // Force repaint for smoother visuals
                if (Event.current.type == EventType.Repaint) { /* placeholder if needed */ }
            }
