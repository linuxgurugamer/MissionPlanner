...
namespace MissionPlanner
{
    ...
    // KSP1 (Unity IMGUI) – hierarchical list of Steps with a separate detail editor window.
    //
    // Requirements implemented:
    // 1) Only the title is shown by default in the main list.
    // 2) Data is edited in a separate "Step Details" window.
    // 3) Steps have levels (nested tree) with Up/Down/Promote/Demote/Move…/Add Child controls.
    //
    ...
    // Tree node to hold the Step plus hierarchy metadata
    [Serializable]
    ...
    public class HierarchicalStepsWindow : MonoBehaviour
    {
        ...
        // Styles
        private GUIStyle _titleLabel, _selectedTitle, _smallBtn, _hintLabel, _tinyLabel, _titleEdit;
        private void EnsureStyles()
        {
            if (_titleLabel == null)
            {
                _titleLabel = new GUIStyle(HighLogic.Skin.button)
                {
            ...
            if (_titleEdit == null)
            {
                _titleEdit = new GUIStyle(HighLogic.Skin.textField)
                {
        ...
        private void DrawTreeWindow(int id)
        {
            ...
            GUILayout.Label($"Count: {CountAll()}", _tinyLabel);
            ...
            GUILayout.Space(6);
            GUILayout.Label("Only the step titles are shown below. Double-click a title or click “Edit…” to modify data in a separate window. Use ▲ ▼ ⤴ ⤵ Move… ⊕ to manage hierarchy.", _hintLabel);
        ...
        // ---- Details Window (all data editing lives here) ----
        ...
        // ---- Sample data (shows nested levels) ----
        ...
        // ---- Details: numeric conversions done in helpers above ----
