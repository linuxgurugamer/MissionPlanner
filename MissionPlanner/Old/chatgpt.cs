...
// KSP1 (Unity IMGUI) + ApplicationLauncher toolbar button.
// Drop the compiled DLL into: GameData/YourMod/Plugins/
// Optional icons (recommended):
//   GameData/YourMod/Icons/tree_on.png
//   GameData/YourMod/Icons/tree_off.png
// (Reference them below as "YourMod/Icons/tree_on" etc. — no file extension)
...
namespace MissionPlanner
{
    ...
    public class RecursiveCollapsibleList : MonoBehaviour
    {
        ...
        private int _treeWinId, _detailWinId;
        ...
        // Node open in the details window
        ...
        // Optional: hotkey to show/hide window
        ...
        public void Start()
        {
            // If launcher is already up (e.g., scene reload), add our button
        ...
        private void DrawTreeWindow(int id)
        {
            ...
            GUILayout.Space(6);
            GUILayout.Label("Click +/− to expand/collapse. Double-click a name to open its text window.", HighLogic.Skin.label);
        ...
        private void DrawNodeRecursive(Node node, int depth)
        {
            ...
            // Label with double-click detection
            var labelClicked = GUILayout.Button(node.Name, HighLogic.Skin.button, GUILayout.ExpandWidth(true));
            if (labelClicked)
            {
        ...
        private void OnAppLauncherReady()
        {
            ...
            _appButton = ApplicationLauncher.Instance.AddModApplication(
                onTrue: OnToolbarToggleOn,      // user toggles ON  -> show window
                onFalse: OnToolbarToggleOff,     // user toggles OFF -> hide window
                onHover: null,
                onHoverOut: null,
                onEnable: null,
                onDisable: null,
            ...
            SyncToolbarState(); // ensure button state matches current visibility
        }
        ...
        {
            if ( /* _appButton?.icon != null && */ tex != null)
