...
//
// This version:
// - Runtime sliders for Title Width %, Controls Pad, Indent/Level, Fold Column Width (persisted in UI.cfg)
// - **Reset** button restores those slider values to defaults (and saves immediately)
// - **Auto-open details** when adding a new item (Add Root or ⊕)
    ...
    public class HierarchicalStepsWindow : MonoBehaviour
    {
        ...
        private void DrawTreeWindow(int id)
        {
            ...
            if (GUILayout.Button("Add Root", GUILayout.Width(100)))
            {
                ...
                OpenDetail(newRoot); // auto-open details for new item
            }
            ...
            if (GUILayout.Button("Reset", GUILayout.Width(80)))
            {
                ...
                _indentPerLevel = 18f;
                _foldColWidth = 24f;
                SaveUISettings(); // persist immediately after reset
            }
        ...
        private void DrawNodeRow(StepNode node, int depth)
        {
            ...
            if (add)
            {
                ...
                node.Expanded = true;
                _selectedNode = child;
                OpenDetail(child); // auto-open details for new child
            }
            if (del) Delete(node);
        ...
        {
            // Auto-save if switching away from a different node
        ...
        private void DrawDetailWindow(int id)
        {
            ...
            {
                TrySaveToDisk();  // Auto-save when details are closed
                _detailNode = null;
            }
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();
