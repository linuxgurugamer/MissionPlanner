...
namespace MissionPlanner
{
    ...
    // KSP1 (Unity IMGUI) – recursive tree with reliable drag & drop (hotControl),
    // toolbar toggle, double-click details, per-node add dialog, hover-to-expand,
    // drop before/after/as-child. Single click on label now ONLY selects (no toggle).
    ...
    namespace YourModNamespace
    {
        ...
        public class RecursiveCollapsibleList : MonoBehaviour
        {
            ...
            private Vector2 _mouseDownPos;  // where the mouse went down on the row
            private bool _pendingClick;     // true until movement exceeds threshold
            private int _activeRowControl;  // the control id that owns the mouse
            ...
            public void OnGUI()
            {
                ...
                // global mouse-up to finalize drop if needed
                HandleGlobalMouseUp();
            ...
            private void DrawNodeRecursive(Node node, int depth)
            {
                ...
                // Full row rect = union of fold + label + add
                Rect rowRect = UnionRects(UnionRects(foldRect, labelRect), addRect);

                HandleRowMouse(node, rowRect, labelRect);
            ...
            // Mouse logic per row (hotControl-based)
            private void HandleRowMouse(Node node, Rect rowRect, Rect labelRect)
            {
                var e = Event.current;
                int id = GUIUtility.GetControlID(FocusType.Passive, rowRect);
                ...
                switch (e.rawType)
                {
                    ...
                    case EventType.MouseDrag:
                        ...
                        {
                            if (_pendingClick && (e.mousePosition - _mouseDownPos).sqrMagnitude > DragThresholdSqr)
                            {
                                // start actual drag
                                _dragNode = node;
                                _pendingClick = false;
                            ...
                            if (_dragNode != null)
                            {
                                // Compute drop intent on this row
                    ...
                    case EventType.MouseUp:
                        if (GUIUtility.hotControl == id && _activeRowControl == id && e.button == 0)
                        {
                            ...
                            {
                                // Finish drop if we have a target
                                if (_dragNode != null && _dragTarget != null && _dragKind != DropKind.None)
            ...
            // Global mouse-up (safety net: e.g., mouse released outside window)
            private void HandleGlobalMouseUp()
            {
                ...
                if (e.rawType == EventType.MouseUp && e.button == 0)
                {
                    if (_dragNode != null)
                    {
                        // no valid drop target → just cancel drag
                        _dragNode = null;
                        _dragTarget = null;
                        _dragKind = DropKind.None;
                        _hoverNode = null;
