...
//
// This version:
// - Added StepType.crewCount (int range) and StepType.part (with part picker dialog)
// - Step has fields for part selection (partName, partTitle, partOnlyAvailable)
// - Part picker dialog: filter by “available only” or “all parts”, with search
// - Details window: Title field fixed width; Type arrows packed (no far-right arrow)
// - Keeps previous features: runtime UI sliders + reset, auto-open detail on add, auto-save on detail close/switch, save/load with overwrite + auto-increment, red/blue save indicator, persisted UI + positions
//
...
namespace MissionPlanner
{
    ...
    public class Step
    {
        ...
        // ---- NEW: Part selection backing fields ----
        public string partName = "";     // internal name (AvailablePart.name / part.cfg "name")
        public string partTitle = "";    // display title (AvailablePart.title)
        public bool partOnlyAvailable = true; // default: only show available parts
        ...
        public static Step FromConfigNode(ConfigNode n)
        {
            ...
            if (bool.TryParse(n.GetValue("partOnlyAvailable"), out bool po)) s.partOnlyAvailable = po;
    ...
    public class HierarchicalStepsWindow : MonoBehaviour
    {
        ...
        private Rect _partRect = new Rect(680, 140, 520, 560); // NEW Part picker

        private int _treeWinId, _detailWinId, _moveWinId, _loadWinId, _overwriteWinId, _deleteWinId, _partWinId;
        ...
        private void DrawTreeWindow(int id)
        {
            ...
            // --- Bottom bar: Save / Load / Hide + Save indicator ---
            GUILayout.Space(6);
            GUILayout.BeginHorizontal();
        ...
        private void DrawNodeRow(StepNode node, int depth)
        {
            ...
            const float controlsBase = 314f; // ▲ ▼ ⤴ ⤵ Move… Dup ⊕ ✖ + margins
            ...
            if (add)
            {
                ...
                node.Expanded = true;
                _selectedNode = child;
                OpenDetail(child); // auto-open details
            }
            if (del) Delete(node);
        ...
        {
            if (_detailNode != null && _detailNode != node) TrySaveToDisk();
        ...
        private void DrawDetailWindow(int id)
        {
            ...
            s.title = GUILayout.TextField(s.title ?? "", _titleEdit, GUILayout.Width(320)); // FIXED WIDTH
            GUILayout.FlexibleSpace();
            ...
            // Type selector — pack arrows & label, no huge gap
            GUILayout.BeginHorizontal();
            ...
            switch (s.stepType)
            {
                ...
                case StepType.crewCount: // NEW — just uses int range UI
                ...
                case StepType.part: // NEW — part selection
                    GUILayout.BeginHorizontal();
        ...
        private void DrawPartPickerWindow(int id)
        {
            ...
            // Top row: available-only toggle + search box
            GUILayout.BeginHorizontal();
            ...
            if (list != null)
            {
                foreach (var ap in list)
                {
                    ...
                    // Filter by search text (title or internal name)
                    ...
                    if (GUILayout.Button("Choose", GUILayout.Width(80)))
                    {
                        ...
                        _showPartDialog = false;
                        _partTargetNode = null;
                        TrySaveToDisk(); // persist selection
                    }
                    GUILayout.EndHorizontal();
                }
            }
        ...
        private bool IsPartAvailable(AvailablePart ap)
        {
            try
            {
                // If R&D exists and in a mode with tech limits, honor tech + purchased flags
                if (ResearchAndDevelopment.Instance != null)
                {
                    // Consider tech availability first
                    if (!ResearchAndDevelopment.PartTechAvailable(ap)) return false;

                    // If purchase required, check purchase
                    // Many installs either auto-purchase or ignore; this call is safe.
                    return ResearchAndDevelopment.PartModelPurchased(ap);
                }
                // Sandbox / no RnD — everything is available
                return true;
            }
            catch
            {
                // If anything goes wrong, be permissive in non-RnD
                return true;
            }
        }
        ...
        private bool TrySaveToDisk()
        {
            ...
            {
                return TrySaveToDisk_Internal(overwriteOk: true);
            }
        }
        ...
        private bool TrySaveToDisk_Internal(bool overwriteOk)
        {
            try
            {
                ...
                ShowSaveIndicator($"Saved ✓ {mission} @ {DateTime.Now:t}", true);
                return true;
            }
            catch (Exception ex)
            {
