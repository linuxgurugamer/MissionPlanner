...
namespace MissionPlanner
{
    ...
    public class HierarchicalStepsWindow : MonoBehaviour
    {
        ...
        private const float _resizeHandleSize = 18f;
        private const float _minTreeW = 720f;
        ...
        private void DrawTreeWindow(int id)
        {
            ...
            GUILayout.TextArea(string.IsNullOrEmpty(_missionSummary) ? "(no summary)" : _missionSummary, GUILayout.ExpandHeight(true));
            ...
            {
                Rect rh = new Rect(_treeRect.width - _resizeHandleSize, _treeRect.height - _resizeHandleSize, _resizeHandleSize, _resizeHandleSize);
                // simple visual hint
                GUILayout.BeginArea(rh);
                GUILayout.Label("◢");
                GUILayout.EndArea();

                var ev = Event.current;
                if (ev.type == EventType.MouseDown && rh.Contains(ev.mousePosition))
                {
                    _resizingTree = true;
                    _resizeStartLocal = ev.mousePosition;
                    _resizeStartRect = _treeRect;
                    ev.Use();
                }
                if (_resizingTree && (ev.type == EventType.MouseDrag || ev.type == EventType.MouseMove))
                {
                    Vector2 delta = ev.mousePosition - _resizeStartLocal;
                    _treeRect.width = Mathf.Max(_minTreeW, _resizeStartRect.width + delta.x);
                    _treeRect.height = Mathf.Max(_minTreeH, _resizeStartRect.height + delta.y);
                    ev.Use();
                }
                if (_resizingTree && (ev.type == EventType.MouseUp || ev.rawType == EventType.MouseUp))
                {
                    _resizingTree = false;
                    ev.Use();
                }
