...
namespace MissionPlanner
{
    ...
    public class HierarchicalStepsWindow : MonoBehaviour
    {
        ...
        private Rect _saveAsRect = new Rect(740, 200, 440, 190); // taller if "New" toggle visible
        private Rect _newConfirmRect = new Rect(760, 200, 520, 180);
        private Rect _clearConfirmRect = new Rect(760, 220, 520, 170);

        private int _treeWinId, _detailWinId, _moveWinId, _loadWinId, _overwriteWinId, _deleteWinId, _partWinId, _saveAsWinId, _newConfirmWinId, _clearConfirmWinId;
        ...
        public void OnGUI()
        {
            ...
            if (_showSaveAs)
            {
                _saveAsRect = GUILayout.Window(
                    ...
                    GUILayout.MinWidth(380), GUILayout.MinHeight((_creatingNewMission ? 170 : 150))
                );
            }
            if (_showNewConfirm)
            {
        ...
        private void DrawNewConfirmWindow(int id)
        {
            ...
            if (GUILayout.Button("Save", GUILayout.Width(100)))
            {
                ...
                // proceed to New flow: ask for name, allow sample step toggle
